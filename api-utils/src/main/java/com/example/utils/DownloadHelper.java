...
public class DownloadHelper {
	 ...
	 * TDCS的Etag資料來源網址
	 */
	private static final String TDCS_ETAG_RESOURCE_ROOT = "";
	/**
	 * TDCS的VD資料來源網址
	 */
	private static final String TDCS_VD_RESOURCE_FORMAT = "";
	...
	public static File download(String urlStr, String outputFilePath, String outputFileName) throws IOException {
		...
		Map<String, String> headers = urlStr.contains("ptx.") ? HttpUtils.customHeaderForPTX() : new HashMap<>();
		try (CloseableHttpClient httpClient = (CloseableHttpClient) HttpUtils.createHttpClient();
				CloseableHttpResponse response = httpClient.execute(HttpUtils.toHttpRequest(urlStr, headers))) {
