...
@Slf4j
@Component("LetServiceForLastMonthApiETLStrategy")
public class LetServiceForLastMonthApiETLStrategy extends GeneralApiETLStrategy {
    ...
    public ExtractResult extract(ResourceInfo resourceInfo) throws ResourceException {
        try {
            ...
            Date lastMonth = DateUtils.addMonths(updateTime, -2);
            Date firstDay = DateUtils.getTheFirstDayOfMonth(lastMonth);
            Date lastDay = DateUtils.getTheLastDayOfMonth(lastMonth);
            List<Date> dates = DateUtils.getDateInterval(firstDay, lastDay);
            ...
            SimpleDateFormat sdfForStart = new SimpleDateFormat("yyyy-MM-dd 00:00:00");
            SimpleDateFormat sdfForEnd = new SimpleDateFormat("yyyy-MM-dd 23:59:59");

            for (Date date : dates) {
                String sDateStr = sdfForStart.format(date);
                String eDateStr = sdfForEnd.format(date);
