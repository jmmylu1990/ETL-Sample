...
public class ETLJobExecutor extends AbstractJobExecutor {
	...
	public void executiveCore(JobExecutionContext context) throws JobExecutionException {
		...
		try {
			...
			ETLStrategy etlStrategy = etlStrategyFactory.getObject(jobStrategy);
			...
			long totalImportCount = importResults.stream()
				...
				.sum();
			List<Exception> exceptionList = importResults.stream()
				.map(ImportResult::getException)
				.filter(Objects::nonNull)
				.collect(Collectors.toList());
			if (!exceptionList.isEmpty()) {
				String failedDbSources = importResultMap.keySet().stream().map(DbSourceEnum::getName).collect(Collectors.joining(", "));
				String failedMessages = exceptionList.stream().map(Exception::getMessage).collect(Collectors.joining("<br>"));
				throw new ImportException(String.format("排程【%s】 資料匯入異常(資料源: %s)<br>例外細節: %s", jobName, failedDbSources, failedMessages));
			}
			long importDataCount = totalImportCount / dbSources.length;
			log.info("排程【{}】資料匯入數: {}", jobName, importDataCount);
			if (importDataCount == 0) {
				throw new ResourceNotUpdateException("Record affected is 0. Maybe the resource has not update.");
			}
			etlStrategy.successCallback(); // Execute callback when job is success
			...
			if (refireCount >= refireMaxCount) {
				...
				log.info("排程【{}】{}累積次數: {}", jobName, srcNotUpdate ? "未更新" : "錯誤", errorAccumulation);
			...
			log.error(e.getMessage(), e);
			scheduleLog.setResult(ETLResultEnum.IMPORT_FAIL);
			scheduleLog.setCallbackMsg(e.getMessage());
			notifyService.failedNotify(
				notifyArgBuilder
					.apistatus(ETLResultEnum.IMPORT_FAIL)
					.exception(e)
					.errorAccumulation(++errorAccumulation)
					.notifyMails(this.fetchVaildNotifyMails(jobNotifyUsers, ETLResultEnum.IMPORT_FAIL))
					.build()
			);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			scheduleLog.setResult(ETLResultEnum.UNKNOWN_ERROR);
			scheduleLog.setCallbackMsg(e.getMessage());
			notifyService.failedNotify(
				notifyArgBuilder
