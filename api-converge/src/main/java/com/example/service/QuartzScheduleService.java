...
import com.example.job.ApiTriggerJobExecutor;
import com.example.job.ETLJobExecutor;
import com.example.job.SQLJobExecutor;
import com.example.job.ShellJobExecutor;
import com.example.model.JobDataMapConstant;
import com.example.model.entity.base.AbstractScheduleJob;
import com.example.model.enums.JobGroupEnum;
...
@Service
@Transactional
public class QuartzScheduleService<T extends AbstractScheduleJob> {
	...
	@Autowired
	private GenericScheduleJobRepository<T> scheduleJobRepository;
	
	// Initial the scheduler instance
	...
	public List<T> listAllJob() {
		return scheduleJobRepository.findAll();
	}

	@SuppressWarnings("unchecked")
	public List<AbstractScheduleJob> dispatcher(AbstractScheduleJob... jobs) {
	...
	public List<AbstractScheduleJob> dispatcher(String jobGroup, String... jobIds) {
		List<T> jobList = scheduleJobRepository.findByJobGroup(jobGroup);
		List<String> jobIdList = Arrays.asList(jobIds);
		AbstractScheduleJob[] matchJobs = jobList.parallelStream()
				.filter(job -> jobIdList.isEmpty() || jobIdList.contains(job.getJobId()))
				.toArray(AbstractScheduleJob[]::new);
		
		return this.dispatcher(matchJobs);
	}
	
	public boolean isJobPresent(AbstractScheduleJob scheduleJob) {
		try {
	...
	public boolean isTriggerPresent(AbstractScheduleJob scheduleJob) {
		try {
			return scheduler.checkExists(
	...
	public void triggerNow(String jobId, boolean createIfNotExisted) throws SchedulerException {
		T scheduleJob = scheduleJobRepository.findByJobId(jobId);
	...
	public boolean stopNow(String... jobIds) throws SchedulerException {
		...
		for (JobExecutionContext context : currentlyExecuting) {
			AbstractScheduleJob scheduleJob = (AbstractScheduleJob) context.getMergedJobDataMap().get(JobDataMapConstant.SCHEDULE_JOB);
	...
	@SuppressWarnings("unchecked")
	public int removeFailingRetryJobs(AbstractScheduleJob scheduleJob) {
	...
	public Date refreshJob(AbstractScheduleJob scheduleJob, boolean createIfNoTrigger) throws SchedulerException {
		String jobId = scheduleJob.getJobId();
		String jobGroup = scheduleJob.getJobGroup();
		...
		// Cron expression builder with misfire properties
	...
	public boolean removeJob(AbstractScheduleJob scheduleJob) throws SchedulerException {
		// Check the `FailingJobsGroup` exists or not, if existed then delete it.
		this.removeFailingRetryJobs(scheduleJob);
		String jobId = scheduleJob.getJobId();
		String jobGroup = scheduleJob.getJobGroup();
	...
	private JobDetail determineJobDetail(String jobId, String jobGroup) {
		...
		case ETL_GROUP:
			return JobBuilder.newJob(ETLJobExecutor.class)
				.withIdentity(jobId, jobGroup)
				.build();
		case SHELL_CALLER_GROUP:
			return JobBuilder.newJob(ShellJobExecutor.class)
					.withIdentity(jobId, jobGroup)
					.build();
		case SQL_CALLER_GROUP:
			return JobBuilder.newJob(SQLJobExecutor.class)
					.withIdentity(jobId, jobGroup)
					.build();
