...
public class ApiRewriteComponent {
	...
	public String rewriteForTDCS(String originUrl) {
		String encodeUrl = Base64.getUrlEncoder().withoutPadding()
				.encodeToString(originUrl.getBytes(StandardCharsets.UTF_8));
		String newUrl = String.format("%s/api/etagResource/fetchUriList/%s", apiRewriteProxyDomain, encodeUrl);
		log.info("【URI Rewrite】 {}-> {}", originUrl, newUrl);
